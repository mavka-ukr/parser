...
take_elements: '[' nls te_star='*' nls ']' | '[' nls take_element (nls ',' nls take_element)* (nls ',')? nls ']';
...
args: arg (nls ',' nls arg)* (nls ',')?;
arg: a_value=expr;
named_args: named_arg (nls ',' nls named_arg)*;
named_arg: na_name=identifier '=' na_value=expr;

params: param (nls ',' nls param)* (nls ',' nls p_variadic='.' '.' '.' p_variadic_name=identifier (p_variadic_type=type_value)?)? (nls ',')?
      | p_variadic='.' '.' '.' p_variadic_name=identifier (p_variadic_type=type_value)? (nls ',')?;
